...
| **Supertype/Subtype** | Inheritance — `IsA` relationship |
...
Higher isolation = stronger correctness guarantees but more lock contention and lower throughput. Most apps run at READ COMMITTED or REPEATABLE READ; SERIALIZABLE is the gold standard for correctness-critical code.
...
- **Availability** = % of time the system is up. Three nines (99.9%) ≈ 8.7 hours of downtime per year; five nines (99.999%) ≈ 5 minutes.
...
- **Primary/secondary** — one writer, many readers. Updates flow primary → secondaries.
...
Data can be **structured** (fixed schema), **semistructured** (named elements but variable shape — JSON, XML), or **unstructured** (free text, images).
...
**MySQL** (the DB used throughout this guide's syntax examples)
